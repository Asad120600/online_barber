...
class AdminOrdersPage extends StatelessWidget {
  ...
  Future<void> _updateOrderStatus(BuildContext context, String orderId, String status, String? token, String userId) async {
    try {
      ...
      // Create notification document in Firestore with userId
      if (message.isNotEmpty && notificationBody.isNotEmpty) {
        await FirebaseFirestore.instance.collection('notifications').add({
          'orderId': orderId,
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final orderStatus = order['status'] ?? 'Pending'; // Default to 'Pending' if status is null
                final userToken = order['deviceToken'] as String?; // Ensure this is handled as a nullable String
                final userId = order['userId'] ?? '';  // Extract the userId
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          const SizedBox(width: 240,),
                          PopupMenuButton<String>(
                            onSelected: (value) => _updateOrderStatus(context, orderId, value, userToken, userId),  // Pass userId
                            itemBuilder: (BuildContext context) {
                              List<String> statusOptions = [];
                              if (orderStatus == 'Pending') {
                                statusOptions = ['Confirmed'];
